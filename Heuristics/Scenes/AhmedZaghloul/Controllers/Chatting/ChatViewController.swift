...
class ChatViewController: JSQMessagesViewController {
    ...
    private lazy var unreadedCountRef: DatabaseReference = Database.database().reference().child("unreaded_messages")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.senderId = (CURRENT_USER?.userId!)!
        ...
        let sendImage = #imageLiteral(resourceName: "send")
...
//            imageView.kf.setImage(with: url, placeholder: #imageLiteral(resourceName: "Logo"), options: [.transition(ImageTransition.fade(1))], progressBlock: { receivedSize, totalSize in
...
//            })
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        observeTyping()
        unreadedCountRef.child((channelRef?.key)!).child(senderId).setValue(true)
    }
    ...
    private func observeMessages() {
        ...
        newMessageRefHandle = messageQuery.observe(.childAdded, with: { (snapshot) -> Void in
            ...
            if let id = messageData["senderId"] as String!, let name = messageData["senderName"] as String!, let text = messageData["text"] as String!, text.count > 0 {
                if self.senderDisplayName != name {
                    self.channelRef!.child("messages").child(snapshot.key).child("is_read").setValue("1")
                }
...
//                    mediaItem.image = UIImage.gifWithData(data!)
    ...
    override func didPressSend(_ button: UIButton!, withMessageText text: String!, senderId: String!, senderDisplayName: String!, date: Date!) {
        ...
        messageRef.parent?.child("last_message").setValue(text!)
        messageRef.parent?.child("date").setValue(date.getStringFromDate())
        
        unreadedCountRef.child((channelRef?.key)!).removeValue()
        let unreadedContent = [
            "\(senderId!)": true,
            "last_message": text!,
            ] as [String : Any]
        unreadedCountRef.child((channelRef?.key)!).setValue(unreadedContent)
...
//        if let photoReferenceUrl = info[UIImagePickerControllerReferenceURL] as? URL {
            ...
            // 2
//            let assets = UIImagePickerControllerPHAsset.fetchAssets(withALAssetURLs: [photoReferenceUrl], options: nil)
